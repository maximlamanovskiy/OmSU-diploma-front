...
function RescheduleEventDialog(props) {
  ...
  const onClassroomChange = obj => {
    ...
    if (obj && reschedule.to) {
      getClassroom(obj.value, reschedule.to, reschedule.to);
    }
  };
  ...
  const handleChangeInDate = event => {
    ...
    if (to && reschedule.classroomId) {
      getClassroom(reschedule.classroomId, to, to);
    }
  };
  ...
  return (
    <DialogWindow className="reschedule-dialog" isOpen={props.isOpen}>
      <DropdownOption
        message={I18n.t('components.labels.building')}
        name="buildings"
        options={buildings}
        textClassName="simple-label__text"
        wrapperClassName="reschedule-event-dialog__drop-down-wrapper"
      ...
      {reschedule.to && reschedule.classroomId ? (
        <DropdownOption
          ...
          name="timeTo"
          options={times}
          textClassName="simple-label__text_small"
          wrapperClassName="reschedule-event-dialog__drop-down-wrapper"
...
const mapStateToProps = state => ({
  ...
  isOpen: state.utilityReducer.isRescheduleDialogOpen,
});
